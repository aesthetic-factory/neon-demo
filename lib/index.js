...
const wasm = require("./wasm-pkg/wasm_pack_demo");
...
const { sha256, sha512, find_prime, fibonacci, blake3_256 } = addon
...
console.time("SHA512-JS")
for (let x = 0; x < LOOP; x++)
    sha512_js(`${x}${TEXT}${x}`);
console.log(`SHA512-JS: ${sha512_js(TEXT)}`)
console.timeEnd("SHA512-JS")

console.time("SHA512-RS")
for (let x = 0; x < LOOP; x++)
    sha512(`${x}${TEXT}${x}`);
console.log(`SHA512-RS: ${sha512(TEXT)}`)
console.timeEnd("SHA512-RS")


console.time("BLAKE3-256-WASM")
for (let x = 0; x < LOOP; x++)
    wasm.blake3_256(`${x}${TEXT}${x}`)
console.log(`BLAKE3-256-WASM: ${wasm.blake3_256(`${TEXT}`)}`)
console.timeEnd("BLAKE3-256-WASM")

console.time("BLAKE3-256-RS")
for (let x = 0; x < LOOP; x++)
    blake3_256(`${x}${TEXT}${x}`);
console.log(`BLAKE3-256-RS: ${blake3_256(`${TEXT}`)}`)
console.timeEnd("BLAKE3-256-RS")
